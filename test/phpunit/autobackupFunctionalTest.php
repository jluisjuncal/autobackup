...
/**
 * \file    test/functional/autobackupFunctionalTest.php
 * \ingroup autobackup
...
/**
 * Class autobackupFunctionalTest
 *
 ...
 *
 * @package Testautobackup
 */
class autobackupFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modautobackup')->click();
		} else {
			// Disable the module
			$this->byHref('modautobackup')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/autobackup/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('autobackup/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/autobackup/admin/about.php');
		$this->authenticate();
		return $this->assertContains('autobackup/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('autobackupwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modautobackup_autobackupTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modautobackup_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
